...
namespace cartservice.cartstore
{
    public class RedisCartStore : ICartStore
    {
        ...
        string cartResponse = Environment.GetEnvironmentVariable("CART_RESPONSE");

        // to keep track of how many times GetCartAsync() has been called
        // by the checkout service
        int getCartRequestCount;

        // bad product id cart
        Hipstershop.Cart badProductIdCart;
        ...
        public RedisCartStore(string redisAddress)
        {
            ...
            getCartRequestCount = 0;

            badProductIdCart = new Hipstershop.Cart();
            cart.UserId = "user";
            cart.Items.Add(new Hipstershop.CartItem { ProductId = "xxxxxdnexx", Quantity = 10 });
        }
        ...
        public async Task<Hipstershop.Cart> GetCartAsync(string userId)
        {
            ...
            Console.WriteLine(getCartRequestCount);

            if (getCartRequestCount >= 1 || cartResponse == "default" || userId != "dummyIDcheckout") {
                Console.WriteLine("GET CART: DEFAULT");
                Console.WriteLine("getCartRequestCount = " + getCartRequestCount);
                try
                {
                    EnsureRedisConnected();
            ...
            } else if (getCartRequestCount == 0 && cartResponse == "exception") {
                Console.WriteLine("GET CART: exception");
                Console.WriteLine("getCartRequestCount = " + getCartRequestCount);
                getCartRequestCount += 1;
                throw new RpcException(new Status(StatusCode.FailedPrecondition, $"some exception"));
            } else if (getCartRequestCount == 0 && cartResponse == "bad product id") {
                Console.WriteLine("GET CART: bad product id");
                Console.WriteLine("getCartRequestCount = " + getCartRequestCount);
                getCartRequestCount += 1;
                return badProductIdCart;
            } else {
                // souldn't get here
                Console.WriteLine("else()");
                throw new RpcException(new Status(StatusCode.FailedPrecondition, "error in mockservice implementation"));
            }
        }
